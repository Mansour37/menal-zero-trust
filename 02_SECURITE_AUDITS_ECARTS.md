...
| H14 | Connexion Elson → Cloud SQL **chiffrée mais non authentifiée** : `ssl: { rejectUnauthorized: false }` sur les 8 instanciations de pool `pg` (`elson-main/backend/src/db.ts:18` pour le chemin runtime, + 7 dans `backend/src/scripts/`), alors que `DB_SSL = "true"` est bien posé en staging (`terraform/environments/staging/elson.tf:32`, instance `ENCRYPTED_ONLY`). Le certificat **serveur** n'est donc jamais vérifié : le chiffrement protège de l'écoute passive, pas d'un MITM actif au sein du VPC — un « vérifier explicitement » manquant au cœur même du discours Zero Trust. L'instance dispose pourtant d'une CA exploitable (`serverCaMode: GOOGLE_MANAGED_INTERNAL_CA`, vérifié en direct le 24/08) | AppSec / chiffrement en transit | Découvert le 24/08 au triage des 74 findings Semgrep de `ci.yml` (règle `bypass-tls-verification`, 6 fichiers) — jamais remonté avant car la gate SAST d'`elson-ci.yml` ne s'exécutait pas (H15) | **Ouvert — assumé et tracé.** Neutralisé en CI par 8 `nosemgrep` **nominatifs** (jamais une exclusion globale : toute nouvelle occurrence ailleurs fait échouer la CI). Correction non tentée avant le tournage vidéo : elle touche la connexion base de l'environnement de démonstration. À corriger en montant la CA de l'instance et en repassant à `rejectUnauthorized: true` |
| H15 | Gate SAST Semgrep d'`elson-ci.yml` silencieusement cassée — **même bug que H12, jamais reporté sur ce workflow**. `semgrep/semgrep-action@v1` tire `returntocorp/semgrep-agent:v1` (semgrep 1.36.0, 2022), qui crashe sur le registre de règles actuel (`ValueError: invalid rule severity value: MEDIUM`) ; l'action avale le code de sortie et l'étape ressort `success`. Le job « Security Scans (Elson) » était donc vert sur **chaque** run sans qu'une seule règle SAST soit évaluée | CI/CD / SAST | Découvert le 24/08 en lisant les logs du run réel #33 (`gh run view --log`) — H12 avait corrigé le même défaut dans `ci.yml` le 19/08, mais `elson-ci.yml` était resté sur l'action morte (corollaire direct de M12, duplication des workflows) | ✅ Résolu le 24/08 : appel direct au CLI Semgrep, **version épinglée** `semgrep==1.173.0`, triage identique à `ci.yml` |
...
| M22 | Actions GitHub référencées par **tag flottant** (`actions/checkout@v4`, `gitleaks/gitleaks-action@v2`, `google-github-actions/auth@v2`…) sur les 4 workflows : un tag peut être redirigé vers un autre commit par le mainteneur (ou un attaquant en cas de compromission), et la CI exécute alors du code différent sans qu'une ligne du dépôt ait changé | CI/CD / supply chain | Identifié le 24/08 au triage Semgrep (règle `github-actions-mutable-action-tag`, 4 occurrences) | Ouvert — règle écartée **nominativement** dans la gate SAST avec justification écrite ; à fermer en épinglant les actions par SHA |
| M23 | Variables GitHub `DASH_DOMAIN` / `DASHBOARD_URL` restées sur `dash-staging.menal-sarl.com` après la bascule vers `dashboard.menal-sarl.com` : le domaine **ne résout plus**. Le smoke test dashboard (`ci.yml:284`, `exit 1` sur statut `000`) et les tests E2E (`ci.yml:320`, `e2e.yml:28`) visaient donc une URL inexistante — la CI MENAL n'ayant plus tourné sur `main` depuis le 20/08, l'échec n'était pas encore visible | CI/CD / configuration | Découvert le 24/08 en tentant une connexion réelle au dashboard (DNS `dash-staging` = NXDOMAIN, `dashboard` = 200) | ✅ Résolu le 24/08 (`gh variable set`). **Reste à corriger** : `04_EXPLOITATION_DEMO.md:671` documente encore l'ancienne URL dans le guide de démonstration |
| M24 | Porte de déploiement Elson (`vars.ELSON_DEPLOY_ENABLED = false`, `elson-ci.yml:226`) **périmée** : sa condition de levée (`elson_enabled = true` côté Terraform) est remplie depuis le 07/08 et les 3 ressources Cloud Run existent. Depuis, chaque push sur `main` construit, scanne et pousse les images Elson sur Artifact Registry **puis s'arrête** — ni migration de schéma, ni déploiement, ni smoke test. Ce qui tourne en staging dérive silencieusement de `main`, et le garde-fou `--wait` sur `elson-migrate` (censé faire échouer le pipeline si une migration casse) ne s'exécute jamais | CI/CD | Identifié le 24/08 en analysant le job sauté du run #33 | Ouvert — bascule volontairement différée à après le tournage vidéo : l'activer redéploie l'environnement exact validé par le GO/NO-GO du 22/08 |
