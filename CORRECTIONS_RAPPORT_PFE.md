...
| C5 | 8 étapes CI séquentielles, 3 portes | **Le rapport est devenu exact le 07/09/2026** : `app-delivery.yml` a été réorganisée en **8 étapes enchaînées** (`needs`) — authentification fédérée, secrets, code, tests, construction d'image, analyse d'image, publication, déploiement — dont **4 portes bloquantes** (Gitleaks, Semgrep, tests, Trivy) | Décrire les 8 étapes et les 4 portes ; préciser que la porte 4 se dédouble (pytest / jest) et les étapes 5-8 par service (api / dashboard). Deux neutralisations restent à assumer : le tag `:latest` poussé en best-effort (`|| true`, requis par Terraform à la création initiale) et la suite E2E **sortie de la chaîne** (elle affichait un job rouge de 12 min non bloquant ; elle reste lançable via `e2e.yml`). Preuves : runs `34165113605` (PR, 7-8 ignorés), `34165407841` (main, déploiement par empreinte) et les 5 runs de blocage du guide de démo. |
